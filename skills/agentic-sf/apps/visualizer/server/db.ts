/**
 * SQLite reader over a target repo's asf.db.
 *
...
const DEFAULT_DB_RELATIVE = "asf/data/asf.db";
const MAX_LIMIT = 1000;
const DEFAULT_LIMIT = 500;

/**
 * Resolve the db path: --db arg wins, then ASF_DB, then <cwd>/asf/data/asf.db.
...
export class AsfDb {
   ...
   * `asf/data/asf.db`), so deriving it as a sibling of the db file keeps
  ...
  constructor(path: string) {
    if (!existsSync(path)) {
      throw new Error(
        `asf.db not found at ${path}\n` +
          `Point the visualizer at a target repo: --db <path> or ASF_DB=<path>, ` +
