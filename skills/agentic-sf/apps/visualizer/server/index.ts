/**
 * asf visualizer server — JSON API over a target repo's asf.db, plus the
 ...
 *   bun run server/index.ts --db /path/to/repo/asf/data/asf.db
 *   ASF_DB=/path/to/asf.db PORT=4600 bun run server/index.ts
 */
import { existsSync, statSync } from "node:fs";
import { join, resolve, sep } from "node:path";
import { AsfDb, resolveDbPath } from "./db.ts";
...
try {
  ...
  console.error(`[asf] ${(error as Error).message}`);
  process.exit(1);
}
...
function safely(
  ...
  return async (req) => {
    try {
      ...
      console.error(`[asf] ${req.method} ${new URL(req.url).pathname}:`, error);
...
async function serveStatic(req: Request): Promise<Response> {
  ...
  if (!existsSync(DIST_DIR)) {
    return new Response(
      `asf visualizer API is running on :${PORT}.\n\n` +
...
console.log(`[asf] visualizer api  http://localhost:${server.port}`);
console.log(`[asf] db              ${db.path}  [journal_mode=${db.journalMode}]`);
console.log(
  existsSync(DIST_DIR)
    ? `[asf] serving ui from  ${DIST_DIR}`
    : `[asf] no ./dist — use "bun run dev" for the Vite dev server on :4601`,
);
